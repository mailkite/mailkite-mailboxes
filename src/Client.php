...
final class Client {
	...
	public static function create_app_password( string $domain, string $local, string $label ) {
		$response = wp_remote_post(
			\MailKite\Smtp\Options::get( 'api_base' ) . '/api/app-passwords',
			[
				'timeout' => 15,
		...
		if ( 2 !== intdiv( $code, 100 ) || empty( $payload['id'] ) || empty( $payload['secret'] ) ) {
			...
			if ( 403 === $code ) {
				$detail = __( 'the site is connected with a domain-scoped key — reconnect with the account key to mint mailboxes', 'mailkite-mailboxes' );
			}

			return new WP_Error(
				'app_password_failed',
				/* translators: %s: reason from the API. */
				sprintf( __( 'MailKite could not create the mailbox credential: %s', 'mailkite-mailboxes' ), $detail ? $detail : (string) $code )
			);
		}
	...
	public static function delete_app_password( string $id ): void {
		wp_remote_request(
			\MailKite\Smtp\Options::get( 'api_base' ) . '/api/app-passwords/' . rawurlencode( $id ),
			[
				'method'  => 'DELETE',
				'timeout' => 10,
	...
	public static function list_messages( string $secret, string $address, string $mailbox = 'INBOX', ?int $before = null ) {
		...
		$response = wp_remote_get(
			add_query_arg( $args, \MailKite\Smtp\Options::get( 'api_base' ) . '/api/mailbox/messages' ),
			[
				'timeout' => 15,
	...
	public static function raw( string $secret, string $address, int $uid, string $mailbox = 'INBOX' ) {
		$response = wp_remote_get(
			add_query_arg(
				...
				\MailKite\Smtp\Options::get( 'api_base' ) . '/api/mailbox/messages/' . (int) $uid . '/raw'
			),
			[
				'timeout' => 20,
				'headers' => [ 'Authorization' => 'Bearer ' . $secret ],
		...
		if ( 2 !== intdiv( (int) wp_remote_retrieve_response_code( $response ), 100 ) ) {
			return new WP_Error( 'mailbox_error', __( 'Could not fetch that message.', 'mailkite-mailboxes' ) );
		}
	...
	public static function set_flags( string $secret, string $address, int $uid, string $flags, string $mailbox = 'INBOX' ): void {
		wp_remote_post(
			add_query_arg(
				...
				\MailKite\Smtp\Options::get( 'api_base' ) . '/api/mailbox/messages/' . (int) $uid . '/flags'
			),
			[
				'timeout' => 10,
				'headers' => [
	...
	private static function site_auth(): array {
		return [ 'Authorization' => 'Bearer ' . (string) \MailKite\Smtp\Options::get( 'api_key' ) ];
	}
	...
	private static function decode( $response ) {
		...
		if ( 2 !== intdiv( $code, 100 ) || ! is_array( $payload ) ) {
			...
			return new WP_Error(
				'mailbox_error',
				/* translators: %s: reason from the API. */
				sprintf( __( 'MailKite returned an error: %s', 'mailkite-mailboxes' ), $detail ? $detail : (string) $code )
			);
		}
